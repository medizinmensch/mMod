...
// see the various methods below on how to change these
  ...
  this.plate = {
    ...
    // TODO: changed size: 200
    size: 0,                 // plate size (X and Y)
  // minor grid settings
    m: {
...
OpenJsCad.Processor.prototype = {
  ...
  // clear previous solid and settings
    ...
    this.clearViewer();
    this.enableItems();
    this.setStatus("Rendering. Please wait <img id=busy src='images/loading.gif'>");
  // rebuild the solid
  ...
  createParamControls: function() {
    ...
    for(var i = 0; i < this.paramDefinitions.length; i++)
    {
      ...
      // TODO: added bezier parameter here
      if (paramdef.type === "bezier") {
        var elem = {
          paramType: "bezier",
        ...
        if (!paramdef.initial) {
          throw new Error("Parameter definition ("+definition.index+ ") must include a 'initial' parameter");
        }

        var bezier_control;
        if (paramdef.container_id) {
          bezier_control = new BezierControl(paramdef.initial, $("#"+paramdef.container_id));
        } else {
          bezier_control = new BezierControl(paramdef.initial);
        }
        ...
        bezier_control.subscribe("mousedrop", callback);
        bezier_control.subscribe("point_removed", callback);

        elem.value = bezier_control;
        this.paramControls.push(elem);
        continue;
      }
      
	  // TODO: added pixel parameter here
      if (paramdef.type === "pixel") {
        var elem = {
          paramType: "pixel",
          paramName: paramdef.name
        };
		...
		var pixelgrid;
        if (paramdef.container_id) {
          pixelGrid.init(paramdef.patternsize,20,paramdef.initial);
          pixelgrid = pixelGrid;
        } 
        
        var draw = document.getElementById("pixelgrid");
        var that = this;
        var clear = document.getElementById("clearButton");
        draw.onclick = function(c){
		      	that.rebuildSolid();
		    	// console.log(pixelgrid)
		    }
        clear.onclick = function (c) {
           that.rebuildSolid();
        }

        elem.value = pixelgrid;
        this.paramControls.push(elem);
        continue;
      }
