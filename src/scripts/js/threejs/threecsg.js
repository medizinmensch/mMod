/*
	...
	Updated to be compatible with current three.js and csg.js versions (Jakob Köhler)
*/
	...
	toCSG: function ( three_model, offset, rotation ) {
		...
		for ( i = 0; i < geometry.faces.length; i++ ) {
			if ( geometry.faces[i] instanceof THREE.Face3 ) {
				vertices = [];
                vertices.push( new CSG.Vertex(new CSG.Vector3D(geometry.vertices[geometry.faces[i].a].clone().add( offset ).applyMatrix4(rotation_matrix)), [ geometry.faces[i].normal.x, geometry.faces[i].normal.y, geometry.faces[i].normal.z ] ));
                vertices.push( new CSG.Vertex(new CSG.Vector3D(geometry.vertices[geometry.faces[i].b].clone().add( offset ).applyMatrix4(rotation_matrix)), [ geometry.faces[i].normal.x, geometry.faces[i].normal.y, geometry.faces[i].normal.z ] ));
                vertices.push( new CSG.Vertex(new CSG.Vector3D(geometry.vertices[geometry.faces[i].c].clone().add( offset ).applyMatrix4(rotation_matrix)), [ geometry.faces[i].normal.x, geometry.faces[i].normal.y, geometry.faces[i].normal.z ] ));
